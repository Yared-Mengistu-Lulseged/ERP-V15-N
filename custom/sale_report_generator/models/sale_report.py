...
class SaleReportGenerator(models.Model):
    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            query = '''
                        select stock_warehouse.name as warehouse,count(so.id),sum(sale_order_line.product_uom_qty) as qty,
                            sum(sale_order_line.price_total) as amount from sale_order as so 
                            inner join sale_order_line on so.id = sale_order_line.order_id
                            INNER JOIN 
                            stock_warehouse ON so.warehouse_id = stock_warehouse.id
                            WHERE 
    ...
    def get_sale_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        if filters.get('report_type') == 'report_by_state':
            ...
            sheet.write('A7', 'Ware House', heading)
            ...
            for rec_data in report_data_main:
                ...
                two_lst = []
                row += 1
                sheet.write(row, col + 1, rec_data['warehouse'], txt_l)
